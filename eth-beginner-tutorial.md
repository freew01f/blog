...
> 本文欢迎转载，转载请标明出处
>
> freewolf 资深IT从业者，关注`微服务`、`区块链`、`敏捷开发`、`前端技术`等，不是大神，只是出于热爱。
>
> 发布时间 `BiTCoin #480499`

## 写在前面

前一段时间到现在一直关注区块链的相关知识，今天本来想帮助小伙伴建立一个基于以太坊的智能合约Demo，发现很多过去的文档都已经过时了，无法正常工作。
...
创建自己的以太坊私有链很简单，首先先建立自己的创世区块描述。新建一个目录，然后创建`genesis.json`文件。
...
> 为什么自己创建创世区块，如果使用默认值，`difficulty`值非常高，挖矿要急死人的。

首先创建两个账户，本文后面需要用到这两个账户，需要输入两次密码。
...
第一个账户没有钱，`accounts[0]`默认情况是`coinbase`账户，也就是挖矿的收益归集账户，那我们来挖个矿。
...
- `miner.start(2)` 开始挖矿，参数是参与计算的线程数
...
5个以太币在`accounts[0]`，现在转2个给`accounts[1]`，转账时候，单位是`wei`，但是注意，既然转账，别忘先解锁账户`accounts[0]`
...
下面我们来创建一个极简单的智能合约，`geth 1.6`编译智能合约，没什么简单的办法，`browser-solidity`是个不错的在线编译选择，我们还是在本地进行操作，前面已经通过`brew`安装了`solidity`，先创建一个`hello.sol`智能合约文件
